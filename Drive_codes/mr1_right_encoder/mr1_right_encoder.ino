  ...
  if (PS4.getButtonClick(TRIANGLE)) {
    if (setpoint >= 0) {
      ...
      while (y <= 180) {
        ...
        sensors_event_t event;
        bno.getEvent(&event);
        y = event.orientation.x;// made change here ask for what does manual do if we click triangle
      }//setpoint to y
    }
    else {
      sensors_event_t event;
