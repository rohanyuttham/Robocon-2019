...
void loop() {
  ...
  int acc1 = 100;
if (check == false) {
  if (myEnc.read() < 63386 && stop_motor==false) {

    if (hcsr04l.distanceInMillimeters() > dist) {

      while (hcsr04l.distanceInMillimeters() > dist1) {
      ...
      while (hcsr04r.distanceInMillimeters() > dist1) {
        Serial.println("2");
        if (myEnc.read() < 63386) {
  ...
  if (PS4.connected()) {
    base_speed = constrain(base_speed + accM, 0, max_speedM); //max and min value of speed
