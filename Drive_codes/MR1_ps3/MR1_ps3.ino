...
void loop() {
  ...
  if (PS3.PS3Connected) {
    ...
    while (check == true) {
      ...
      if (abs(myEnc.read()) < 57000 && stop_motor == false) {
        Serial.println(myEnc.read());
        ang_speed = 9000;
        ang_speed1 = 7000;
        if (hcsr04r.distanceInMillimeters() > dist || (abs(myEnc.read())>12000 && abs(myEnc.read())<49000)) {
         Serial.println("in loop");
          while (hcsr04r.distanceInMillimeters() > dist1 || (abs(myEnc.read())>12000 && abs(myEnc.read())<49000)){
          ...
          if (temp == 0 || abs(myEnc.read()) <10756 ) {

            while (abs(myEnc.read()) <10756) {
          ...
          while (hcsr04l.distanceInMillimeters() > 350) {

            //Serial.println("2");


            if (abs(myEnc.read()) < 57000) {
              ...
              int* p4 = move_angl(ang_speed1, angle_l);

              int* p5 = move_angr(ang_speed1, angle_r);
          ...
          if (temp1 == 0) {
            ...
            while (abs(myEnc.read()) < 35533) {

              //Serial.println(myEnc.read());

              base_speed = constrain(base_speed + acc, 0, 3000); //max and min value of spee
